...
demosaicking_process = Demosaic()
valid_lpips = LPIPS(boundary_ignore=10)
...
class BAENet(pl.LightningModule):
    ...
    def forward(self, x1, x2, x, meta_info, total_exp_time, oft_flag=False):
...
class BurstSimulator(pl.LightningModule):
    ...
    def forward(self, pred_exps, irr_seq, meta_info, batch_idx):
        ...
        for i in range(len(self.exposures)-1):
            ...
            curr_irrs *= onehot[onehot_IDX].view(1,curr_irrs.shape[1],1,1,1) # relaxation
            ...
            scale_var = read_noise + shot_noise * curr_irrs # Add noise
            ...
            self.raws[:,i] = self.pixel_unshuffle(curr_irrs) # pixel-unshuffle (1ch to 4ch)
    ...
    def demosaic_BracketIRE(self, input_raw, batch_idx=0):
        
        bayer = torch.pow(input_raw, 1 / 2.2)
        if batch_idx == 0:
            pretrained_model_path = "./isp/model.bin"
            self.demosaic_net = demosaic_bayer.get_demosaic_net_model(pretrained=pretrained_model_path, device=input_raw.device, cfa='bayer', state_dict=True)
        rgb = demosaic_bayer.demosaic_by_demosaic_net(bayer=bayer, cfa='RGGB', demosaic_net=self.demosaic_net, device=input_raw.device)
        output_rgb = torch.pow(rgb.clip(0,1), 2.2)
...
class Burstormer(pl.LightningModule):
    def __init__(self, args=None, mode='color', num_features=48, burst_size=8, reduction=8, bias=False):
        ...
        self.rec_conv1 = nn.Sequential(nn.Conv2d(num_features*4, num_features*2, kernel_size=1, stride=1, padding=0, bias=False), nn.GELU())
    ...
    def predict_step(self, predict_batch, batch_idx):
        ...
        if self.args.visualize:
            res_path = os.path.join("./results/%s/visual_results/%s/%04d" %(meta_info['exp_name'][0], meta_info['clip_name'][0], batch_idx))
            if not os.path.exists(res_path):
                os.makedirs(res_path, exist_ok=True)

            prev_vis = pred.detach().clone().squeeze()
            prev_vis = rgb2raw.gamma_compression(prev_vis)*255+0.5
            prev_vis = prev_vis.permute(1,2,0).detach().cpu().numpy().astype(np.uint8)
            cv2.imwrite(os.path.join(res_path, "prev.png"), cv2.cvtColor(prev_vis, cv2.COLOR_RGB2BGR))
        ''' ============================================================================ ''' 

        pred_exps, optflow_val = self.baenet(pred1, pred2, pred, meta_info, irr_seq.shape[1]-1-7*3, oft_flag=True)
        x = self.burstsimulator(pred_exps, irr_seq, meta_info, batch_idx)

        if batch_idx == 0:
            demosaicking_process.cuda()
            valid_lpips.to(y.device)

        ''' ============================= visualize (bursts) ============================= '''
        if self.args.visualize:
            for i in range(4):
                burst_vis = x[:,i].detach().clone()
                burst_vis = pixel_shuffle(burst_vis)
                burst_vis = demosaicking_process(burst_vis)[0]
                burst_vis = rgb2raw.apply_gains(burst_vis, meta_info['rgb_gain'], meta_info['red_gain'], meta_info['blue_gain'], clamp=True)
                burst_vis = rgb2raw.apply_ccm(burst_vis, meta_info['cam2rgb'].squeeze(0))
                burst_vis = rgb2raw.gamma_compression(burst_vis).clip(0,1)
                burst_vis = (burst_vis * 255. + 0.5).permute(1,2,0).detach().cpu().numpy().astype(np.uint8)
                cv2.imwrite(os.path.join(res_path, "burst_%d.png" %(i)), cv2.cvtColor(burst_vis, cv2.COLOR_RGB2BGR))
        ''' =============================================================================== '''
        ...
        PSNR = self.valid_psnr(pred, y)
        SSIM = structural_similarity(pred[:,:,10:-10,10:-10].squeeze(0).cpu().numpy(), y[:,:,10:-10,10:-10].squeeze(0).cpu().numpy(), channel_axis=0, gaussian_weights=True, use_sample_covariance=False, data_range = 1.0, full=False)

        pred_ps = pixel_shuffle(pred)
        pred_rgb = demosaicking_process(pred_ps).float().clamp(0, 1.)
        y_ps = pixel_shuffle(y)
        y_rgb = demosaicking_process(y_ps).clamp(0, 1.)
        
        LPIPS = valid_lpips(pred_rgb, y_rgb)

        ## save result image
        ''' ============================= visualize (restored) ============================= '''
        if self.args.visualize:
            vis_mosaic = pred
            vis_mosaic = pixel_shuffle(vis_mosaic)
            vis_rgb = self.burstsimulator.demosaic_BracketIRE(vis_mosaic, batch_idx)[0]
            vis_rgb = rgb2raw.apply_gains(vis_rgb, meta_info['rgb_gain'], meta_info['red_gain'], meta_info['blue_gain'], clamp=True)
            vis_rgb = rgb2raw.apply_ccm(vis_rgb, meta_info['cam2rgb'].squeeze(0)) 
            vis_rgb = rgb2raw.gamma_compression(vis_rgb).clip(0, 1)
            vis_rgb = (vis_rgb * 255. + 0.5).permute(1, 2, 0).cpu().numpy().astype(np.uint8)
            vis_rgb = cv2.cvtColor(vis_rgb, cv2.COLOR_BGR2RGB)
            cv2.imwrite(os.path.join(res_path, 'restored.png'), vis_rgb)
        ''' ================================================================================= '''

        ## save gt image
        ''' ============================= visualize (gt & meta_info) ============================= '''
        if self.args.visualize:
            vis_mosaic = y
            vis_mosaic = pixel_shuffle(vis_mosaic)
            vis_rgb = self.burstsimulator.demosaic_BracketIRE(vis_mosaic, batch_idx)[0]
            ...
            vis_rgb = (vis_rgb * 255. + 0.5).permute(1, 2, 0).cpu().numpy().astype(np.uint8)
            vis_rgb = cv2.cvtColor(vis_rgb, cv2.COLOR_BGR2RGB)
            cv2.imwrite(os.path.join(res_path, 'gt.png'), vis_rgb)

            ## save meta_info
            meta_info['optflow_val'] = optflow_val
            with open(os.path.join(res_path,'meta_info.txt'), 'w') as f:
                for key, value in meta_info.items():
                    f.write(f"{key}: {value}\n")
        ''' ================================================================================= '''
        
        exp_list = [round(value, 2) for value in self.burstsimulator.exposures.tolist()]
        result = (os.path.join(meta_info['clip_name'][0], "%04d" %(batch_idx % 10000)), (PSNR.item(), exp_list, SSIM.item(), LPIPS.item(), optflow_val))
        return result
