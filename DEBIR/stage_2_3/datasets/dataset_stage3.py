...
class GoProRAW2RAW(torch.utils.data.Dataset):
    def __init__(self, root, split='train', patch_size=256, args=None):
        ...
        self.burst_list = {}
        # gopro dataset
        folder_name = "train1" if split == "train" else "test"
        video_names_ = [name for name in os.listdir(os.path.join(root, folder_name)) if os.path.isdir(os.path.join(root, folder_name))]
        video_names = sorted(video_names_)
        for name in video_names:
            self.burst_list[name] = glob.glob(os.path.join(root, folder_name, name, "*"))
    ...
    def __getitem__(self, index):
        ...
        if name != "realblur": # gopro dataset
            for img_path in self.burst_list[name][k*self.total_exp_time+(57-self.pred_exp_time):(k+1)*self.total_exp_time]:
                ...
                h, w, c = img.shape
                h_off = h//2 + (hi - 1) * self.patch_size
                w_off = w//2 + (wi - 2) * self.patch_size
                if self.split == "train":
        ...
        meta_info = {'rgb2cam': rgb2cam, 'cam2rgb': cam2rgb, 'rgb_gain': rgb_gain, 'red_gain': red_gain, 'blue_gain': blue_gain, 'max_iso': max_iso,
                     'exp_name': self.args.exp_name.split('/')[-1], 'clip_name': name, 'pred_iso': pred_iso, 'split': self.split}
