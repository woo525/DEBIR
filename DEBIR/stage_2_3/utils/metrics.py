...
class SSIM(nn.Module):
    def __init__(self, boundary_ignore=10, data_range=1.0):
        """
        boundary_ignore: SSIM 계산 시 제외할 바운더리 크기 (픽셀 단위)
        """
        super().__init__()
    ...
    def forward(self, pred, gt):
        ...
        # 바운더리 무시: 중앙 영역만 사용
        if self.boundary_ignore > 0:
            b, c, h, w = pred.shape
            bi = self.boundary_ignore
            pred = pred[:, :, bi:h-bi, bi:w-bi]
            gt = gt[:, :, bi:h-bi, bi:w-bi]
...
class LPIPS(nn.Module):
    def __init__(self, boundary_ignore=10, net='vgg'):
        """
        boundary_ignore: LPIPS 계산 시 제외할 바운더리 크기 (픽셀 단위)
        net: 사용할 LPIPS 모델 (기본: 'vgg', 다른 옵션: 'alex', 'squeeze')
        """
        super().__init__()
